...
public class BottomNavigation extends LinearLayout {
    ...
    private class ScrollAnimation {
        ...
        public void handleScroll(int dy) {
            ...
            AnimatorListenerCompat.withEnd(mViewPropertyAnimator, new Runnable() {
    ...
    @NonNull public ConfigurationBuilder builder() {
            ...
            itemView.setOnClickListener(new OnClickListener() {
                @Override public void onClick(View view) {
                    ...
                    if (mWithColorChanges) {
                        mBackgroundDrawable.animate((int) view.getX() + view.getWidth() / 2,
                            (int) view.getY() + view.getHeight() / 2, item.color);
                    }
                }
            });
    ...
    private void animateUnselected(View view) {
        ...
        ViewPropertyAnimator textAnimatior =
            text.animate().alpha(0f).translationY(0).translationX(0).setDuration(ANIMATION_DURATION);

        AnimatorListenerCompat.withEnd(textAnimatior, new Runnable() {
            @Override public void run() {
                text.setVisibility(View.INVISIBLE);
            }
        });
        textAnimatior.setInterpolator(new AccelerateInterpolator()).start();

        ViewPropertyAnimator iconAnimator = icon.animate().translationY(0).translationX(0);
        iconAnimator.setDuration(ANIMATION_DURATION);
        AnimatorListenerCompat.withEnd(iconAnimator, new Runnable() {
            @Override public void run() {
    ...
    private void animateSelected(View view) {
        ...
        ViewPropertyAnimator iconAnimator =
            icon.animate().translationY(-mIconTranslationY).translationX(xValue).setDuration(ANIMATION_DURATION);
        AnimatorListenerCompat.withEnd(iconAnimator, new Runnable() {
            @Override public void run() {
